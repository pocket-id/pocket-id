...
const (
	...
	// oneTimeAccessConsumeDeviceMismatch indicates the provided device token doesn't match; the token is left untouched
...
func (a *oneTimeAccessTokenActor) consume(parentCtx context.Context, data actor.Envelope) (oneTimeAccessConsumeResponse, error) {
	...
	// If the token requires a device token, it must match; a mismatch leaves the token untouched, mirroring the pre-actor behavior
