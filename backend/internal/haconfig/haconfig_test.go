...
func TestCheck(t *testing.T) {
	t.Run("records the value on first startup", func(t *testing.T) {
		for _, haEnabled := range []bool{false, true} {
			t.Run("HAEnabled="+strconv.FormatBool(haEnabled), func(t *testing.T) {
				...
				require.Equal(t, strconv.FormatBool(haEnabled), stored)
			})
		}
	})
	...
	t.Run("concurrent first startups converge on a single value", func(t *testing.T) {
		...
		for range parallel {
			go func() {
				<-start
				errCh <- Check(t.Context(), db, true)
			}()
		}
		close(start)

		for range parallel {
			require.NoErrorf(t, <-errCh, "goroutine errored")
		}
