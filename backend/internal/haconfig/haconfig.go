...
//
// On the first startup it atomically records the current value in the "kv" table
// Then, on subsequent startups, it reads back the stored value and compares it with the configured one, returning an error if they differ.
//
// The setting is immutable because it changes how replicas coordinate against the shared database, so switching it under an existing cluster would be unsafe.
// To change it, the data must be exported and re-imported into a new database.
func Check(parentCtx context.Context, db *gorm.DB, haEnabled bool) error {
	...
	// This query is valid for both SQLite and Postgres.
	var stored string
	err := db.
		WithContext(ctx).
		Raw(
