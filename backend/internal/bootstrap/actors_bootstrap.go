...
// The returned host must NOT be Run(): only direct state operations (SetState/GetState/DeleteState) are supported, and they require the actor state tables to already exist, which is the case whenever the server has run at least once against this database.
