...
// The state itself has no TTL; instead, the actor keeps a single alarm scheduled for the moment the earliest-expiring token expires. When that alarm fires, the actor purges every expired token and, if any tokens remain, reschedules the alarm for the next earliest expiration. This replaces the periodic cleanup job.
...
func (s *signupTokenActorState) removeExpired(now time.Time) int {
	var removed int
...
func (s *signupTokenActorState) earliestExpiration() (time.Time, bool) {
