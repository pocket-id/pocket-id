<script lang="ts">
	...
	const formSchema = z
		...
		.superRefine((data, ctx) => {
			const fields: (keyof z.infer<typeof formSchema>)[] = ['smtpHost', 'smtpPort', 'smtpFrom'];

			const anyProvided = fields.some((f) => !!data[f]);
			if (anyProvided) {
				for (const f of fields) {
					if (!data[f]) {
						ctx.addIssue({
							code: 'custom',
							path: [f],
							message: m.smtp_field_required_when_other_provided()
						});
					}
				}
			}
				...
				for (const f of fields) {
					if (!data[f]) {
						ctx.addIssue({
							code: 'custom',
							path: [f],
							message: m.smtp_field_required_when_email_enabled()
						});
					}
				}
			}
